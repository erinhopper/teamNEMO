...
curr.stars.crop <- env.stars.crop %>% sf::st_crop(bbox)

clamp <- TRUE       # see ?predict.maxnet for details
type <- "cloglog"
current.predicted <- predict(sdm.model, curr.stars.crop, clamp = clamp, type = type)

fil <- here::here("data", "models", "current.RData")
save(current.predicted, file=fil)

#plot with occurance
ggplot() +
  geom_stars(data = current.predicted) +
